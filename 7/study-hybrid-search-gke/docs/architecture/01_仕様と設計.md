...
flowchart LR
    ...
    PS1 --> BQ_IMP
    SAPI -.->|user_actions<br/>(click / detail_view / favorite /<br/>request_button_click / request_complete /<br/>inquiry_complete / contract / bounce)| PS2 --> BQ_ACT
...
- **labeling SQL**: BigQuery で `search_impressions` × `user_actions` を join し、重み付き relevance label (`click`=1, `detail_view`=2, `favorite`=3, `request_button_click`=4, `request_complete`=5, `inquiry_complete`=7, `contract`=10, `no_action`=0, `bounce`=0/-1) を計算 → BigQuery `ranking_labels` テーブル
...
- **Event schema 整合性 (v3 設計改訂、2026-05-05)**: Phase 3 PostgreSQL ↔ Phase 4 BigQuery ↔ Phase 7 Composer DAG の **`search_events` / `search_impressions` / `user_actions` の 3 テーブル + `action_type` enum 8 種** (`click` / `detail_view` / `favorite` / `request_button_click` / `request_complete` / `inquiry_complete` / `contract` / `bounce`) **+ 重み付き relevance label** が同一スキーマを保つ。adapter の差し替えだけで Phase を跨ぐ event 経路が成立する状態を `make check` 系で守る
