...
- ⚠️ **ES は学習用 (a') 解で運用**: `infra/manifests/elasticsearch/elasticsearch.yaml` は HTTP + anonymous superuser (`xpack.security.authc.anonymous.*` / `selfSignedCertificate.disabled: true`)。学習プロジェクト前提の意図的設定で、production hardening (HTTPS + password auth) は **active backlog から外し parked**。手順 + 判断記録 + 破綻条件は [`docs/backlog/production-hardening.md`](docs/backlog/production-hardening.md)。contract test `test_es_manifest_pins_http_and_anonymous_auth` がこの状態を pin (= 自己拘束)。**商用展開時は必ず `xpack.security.authc.anonymous.*` を無効化すること**
