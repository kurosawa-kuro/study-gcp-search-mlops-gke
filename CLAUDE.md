...
| 3 | `3/study-hybrid-search-local/` | 不動産検索 (Local) **+ 最小行動ログ収集** | ハイブリッド検索 + Port/Adapter (Meilisearch + PostgreSQL + Redis + multilingual-e5) + PostgreSQL event tables (`search_events` / `search_impressions` / `user_actions`) + 弱教師ラベル作成 + LightGBM 再学習用 training dataset 整備 |
| 4 | `4/study-hybrid-search-gcp/` | 不動産検索 (GCP) **+ 検索ログ正解データパイプライン** | GCP マネージドサービス化 (Meilisearch on Cloud Run + BigQuery `VECTOR_SEARCH` + Terraform + WIF) + Cloud Run 構造化ログ → Cloud Logging → GCS raw events → BigQuery `raw_events` / `curated` / `ranking_labels` / `training_dataset` / `evaluation_metrics` (NDCG@10 / Recall@K / CTR / CVR) + Cloud Run Job retrain → GCS model artifact |
| 5 | `5/study-hybrid-search-vertex/` | 不動産検索 (Vertex AI 本番MLOps基盤) | **論理 Phase (個別コード保守なし、Phase 7 完成版コードを参照)**。Vertex AI 本番MLOps プリミティブ (Pipelines / **Feature Store (Feature Group / Feature View / Feature Online Store) (training-serving skew 防止のため必須、PII / user_id は扱わない)** / **Vertex Vector Search (semantic 検索の本番 serving index、embedding 生成履歴・メタデータは BigQuery 側に保持)** / Model Registry / Monitoring v2) を Phase 7 完成版コード (`7/study-hybrid-search-gke/`) で確認しながら学ぶ技術境界 |
| 6 | `6/study-hybrid-search-pmle/` | GCP PMLE + 運用統合ラボ | **論理 Phase (個別コード保守なし、Phase 7 完成版コードを参照)**。Composer 本線昇格 + PMLE 追加技術 (BQML / Dataflow / Explainable AI / Monitoring SLO 等) を Phase 7 完成版コード (`7/study-hybrid-search-gke/infra/terraform/modules/composer/` + `pipeline/dags/` の 3 DAG) で確認しながら学ぶ論理境界。Feature Store / Vertex Vector Search は Phase 5 から継承。不変は「不動産ハイブリッド検索というテーマとその中核コード」のみ |
| 7 | `repo root (旧 7/study-hybrid-search-gke)` | 不動産検索 (GKE + KServe, 到達ゴール = canonical) **+ 継続改善 MLOps サイクル** | **教材コード完成版の到達ゴール / canonical 起点**。**Phase 5 / 6 の技術要素を本 phase 配下に集約** (親 README §4) するため、本 phase に **PMLE 4 技術 + Cloud Composer 本線 orchestration (`infra/terraform/modules/composer/` + 3 DAG) + Vertex AI Feature Store + Vertex Vector Search + Vertex AI Pipelines / Model Registry / BigQuery / Meilisearch を本実装**。Serving 層は Cloud Run → GKE Deployment + KServe InferenceService。Feature Online Store は KServe から **Feature View 経由で** opt-in 参照 (Phase 7 固有) |

Phase 1 -> 2 は「学習基礎」と「アプリ・設計パターン」を分離し、Phase 3 以降で検索ドメインに展開する。**Phase 7 = 到達ゴール / canonical 起点** であり、教材コード完成版は Phase 7 配下にすべて集約 (Composer / PMLE 4 技術 / Feature Store / Vector Search / GKE+KServe)。**Phase 1 / 2 / 3 / 4 = 独立教材コード** (Phase 4 は Phase 3 からの足し算で構築、Vertex AI 以降は含めない)、**Phase 5 / 6 = 論理 Phase (個別コード保守なし、Phase 7 完成版コードを参照)**。詳細は `README.md` §4 「コード化方針」を参照 (2026-05-03 改訂)。
...
- **Event schema 共通契約 (v3 設計改訂、2026-05-05)**: 検索 MLOps の継続改善サイクル (行動ログ → 正解データ → 再学習 → 評価 → deployment gate) を Phase 3 / 4 / 7 で同型に維持するため、以下の 3 テーブル + `action_type` enum + 重み付き relevance label を **Phase 3 PostgreSQL → Phase 4 BigQuery → Phase 7 Composer DAG** で同型に保つ:
  ...
  - `ranking_labels.label_source` カラムに書く文字列値の canonical: `no_action` / `bounce` 短形 ([`docs/architecture/01_仕様と設計.md` §2.1](docs/architecture/01_仕様と設計.md) と統一)
  - ⚠️ **canonical 死守ライン (LightGBM 接続)**: 上記 `ranking_labels` を集めても、**`pipeline/training_job/main.py` から Repository 経由で trainer に届く配線実装が完了していない限り LightGBM 学習に流れない**。Phase 3 Wave 1-4 完了時点では trainer は `synthetic_ranking_frames(seed=42)` で乱数学習しており、`ranking_labels` 未接続。**Phase 3 Wave 7** = `ml/data/loaders/postgres_ranker_repository.py` を新設し `training_job/main.py` から呼ぶ配線実装、**Phase 7** = 既存 `ml/data/loaders/ranker_repository.py` を `pipeline/training_job/main.py` に配線、で初めて Event schema の正解データが LightGBM に届く。詳細は Phase 3 [`docs/tasks/02_移行ロードマップ.md` §0 不変ルール 0 + §5 Wave 7](3/study-hybrid-search-local/docs/tasks/02_移行ロードマップ.md)
...
- Phase 4 (独立コード、Phase 3 からの足し算): クラウド実行基盤 (Cloud Run / Cloud Run Jobs / BigQuery / GCS / Secret Manager / Terraform / WIF)。BQ feature table / view の土台 + 軽量 serverless orchestration (Cloud Scheduler / Eventarc / Cloud Function) で retrain trigger を扱う。**Phase 4 では加えて、Cloud Run 構造化ログ → GCS raw → BigQuery curated → labeling SQL → LightGBM training dataset → GCS model artifact → BigQuery `evaluation_metrics` (NDCG@10 / Recall@K / CTR / CVR) の検索ログ正解データパイプラインを構築** (v3 設計改訂、2026-05-05)。Vertex Model Registry への登録は Phase 7 側に寄せ、Phase 4 では GCS model artifact + BigQuery metrics までで止める。**Vertex AI 以降は Phase 4 では扱わない** (Phase 7 完成版に集約)
- Phase 5 (論理 Phase = 個別コード保守なし): Vertex AI MLOps 化の技術境界を Phase 7 完成版コード (`7/study-hybrid-search-gke/`) を参照しながら学ぶ。Vertex AI Pipelines / Feature Store / Vector Search / Model Registry / Monitoring を Phase 7 で確認
- Phase 6 (論理 Phase = 個別コード保守なし): Composer 本線化 + PMLE 追加技術 (BQML / Dataflow / Explainable AI / Monitoring SLO) の論理境界を Phase 7 完成版コード (`7/study-hybrid-search-gke/infra/terraform/modules/composer/` + 3 DAG) を参照しながら学ぶ。Feature Store / Vertex Vector Search は Phase 5 から継承して同じく Phase 7 完成版で確認
...
この段差は、設計思想を維持したまま実行基盤を段階的に差し替える教材設計のためのもの。**コード自体は Phase 1 / 2 / 3 / 4 / 7 で独立保守し、Phase 5 / 6 は Phase 7 完成版コードを参照する論理 Phase**。**コード化対象 Phase の本実装は Phase 7 = canonical 起点に集約**するのが学習の到達点。
...
```bash
cd 1/study-ml-foundations && make               # コード化対象。ML 基礎 (preprocess / training / evaluation / artifact 保存)
cd 2/study-ml-app-pipeline && make              # コード化対象。Docker Compose 系: build/seed/train/serve/test
cd 3/study-hybrid-search-local && make help     # コード化対象。Wave 1-5 GOAL 達成済 (NDCG@10=0.9822, 2026-05-05)、SYN-1 同義語辞書 + SYN-2 レスポンスキャッシュ完了
cd 4/study-hybrid-search-gcp && make help       # コード化対象 (Phase 3 からの足し算で構築中、Vertex AI 以降は含めない、検索ログ正解データパイプラインを BigQuery 上で構築)
cd 5/study-hybrid-search-vertex && make help    # 論理 Phase (個別コード保守なし、Phase 7 完成版コードを参照)
cd 6/study-hybrid-search-pmle && make help      # 論理 Phase (個別コード保守なし、Phase 7 完成版コードを参照)
cd /home/ubuntu/repos/study-gcp-search-mlops-gke && make help       # canonical 起点。Composer 本実装 + PMLE 4 技術 + Feature Store + Vector Search + GKE + KServe (到達ゴール)
```
