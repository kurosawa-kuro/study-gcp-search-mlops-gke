...
> **Phase 5 からの差分**: `search-api` (Cloud Run → GKE Deployment + Gateway) / encoder / reranker (Vertex Endpoint → KServe `InferenceService`)。Pipelines / **Vertex AI Feature Store (Feature Group / Feature View / Feature Online Store)** (Phase 5 必須) / Model Registry / **Vertex Vector Search** (Phase 5 で BQ `VECTOR_SEARCH` から置換、本 Phase でも継承) / Meilisearch は据え置き。
