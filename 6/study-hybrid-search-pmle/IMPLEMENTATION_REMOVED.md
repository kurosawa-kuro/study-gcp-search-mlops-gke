# Phase 6 実装データは削除済 (2026-05-02)

本 phase (`6/study-hybrid-search-pmle/`) の **実装データ (コード)** は
2026-05-02 に削除された。残っているのは:

- `README.md` — phase 概要 (教育資料)
- `CLAUDE.md` — phase 別作業ガイド
- `docs/` — 仕様書 / 移行ロードマップ / 実装カタログ / 運用 / 教育資料 / `decisions/`
...
```bash
# Phase 6 全体を復元
git checkout pre-phase3to6-impl-removal -- 6/study-hybrid-search-pmle/

# 特定ディレクトリだけ復元 (Composer DAG / PMLE 統合スクリプトなど)
git checkout pre-phase3to6-impl-removal -- 6/study-hybrid-search-pmle/pipeline/
