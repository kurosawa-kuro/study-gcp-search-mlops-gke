# ADR 0005 — canonical 構成 で Deployment env は manifest 側で管理し Terraform は touch しない

**Status**: Accepted
**Phase**: Composer なし派生 起点 → canonical 構成 で構造化

## Context

encoder 期-6 では Cloud Run Service の env を Terraform `google_cloud_run_v2_service` 内に直書き
していた。canonical 構成 で `search-api` を GKE Deployment に移し、Terraform で Deployment の env も
管理しようとすると次の問題が発生:

- canonical 構成 PDCA loop で `kubectl set image` / `kubectl set env` を試行錯誤すると、次の
...
- 1 コミットで「インフラと配備」を atomic 変更できなくなる (Terraform PR + manifest PR が
  順序問題になる) — ただし Composer なし派生 まで Cloud Run でやっていたときに比べて drift 事故が減る
- ESO 用 SA (`sa-external-secrets`) を 1 つ追加する分、`iam` module は膨らむ
- 将来 canonical 拡張時に「Terraform で Deployment まで管理したい」と判断するなら本 ADR を Superseded
