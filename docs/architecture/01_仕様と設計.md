...
## §2. Cloud Composer の位置づけ (本線 orchestration)
...
Cloud Composer / Managed Airflow Gen 3 を **本線オーケストレーター** として `infra/terraform/modules/composer/` / `pipeline/dags/` / `scripts/deploy/composer_deploy_dags.py` / `make composer-deploy-dags` で本実装する。

### 2.1 Composer × Vertex Pipelines は代替ではなく上下関係
