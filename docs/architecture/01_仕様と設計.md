...
**「中核」(Meilisearch / multilingual-e5 / RRF / LightGBM LambdaRank)** は不変。それ以外は adapter 実装を差し替える設計。Local 検証は「壊れていないことを早く落とす場所」、Cloud canonical は「本線として動作する場所」、実案件 reference は「Elasticsearch + 本格運用にスケールする想定構成」。
...
- live 値 (`meili_base_url` / `VERTEX_VECTOR_SEARCH_*` / `VERTEX_FEATURE_*`) は手動 patch なしで overlay できる
