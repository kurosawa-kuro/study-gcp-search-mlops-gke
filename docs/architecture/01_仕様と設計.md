...
- **中核 5 要素の挙動・データフロー・デフォルト `/search` 応答は維持**: Meilisearch BM25 / multilingual-e5 / Vertex AI Vector Search / RRF / LightGBM LambdaRank
...
|---|---|---|---|
| (1a) Lexical | Meilisearch (Docker) | Meilisearch on Cloud Run | **Elasticsearch** |
...
実案件で本番運用する場合の理想構成: **Elasticsearch + Redis 同義語辞書 + multilingual-e5 + Vertex AI Vector Search + RRF + LightGBM LambdaRank**。本リポは Meilisearch を学習用 substitute として据え置き、**Redis 同義語辞書 (query expansion) は本実装** に含む (`SynonymExpanderPort` + `RedisSynonymExpander` + Cloud Memorystore for Redis モジュール `infra/terraform/modules/redis_synonym/`、辞書本体は `definitions/synonyms/real_estate_ja.yaml`)。`enable_redis_synonym=true` で Memorystore を opt-in 起動し、`make sync-synonyms` で辞書を投入する経路。Port/Adapter 構造は維持されているので `MeilisearchAdapter` ↔ `ElasticsearchAdapter` への差し替えだけで実案件 reference architecture に到達できる。
...
**中核挙動への影響**: なし (主経路は Meilisearch + Vertex Vector Search + LightGBM rerank で固定)。
