# Phase 5 実装データは削除済 (2026-05-02)

本 phase (`5/study-hybrid-search-vertex/`) の **実装データ (コード)** は
2026-05-02 に削除された。残っているのは:

- `README.md` — phase 概要 (教育資料)
- `CLAUDE.md` — phase 別作業ガイド
- `docs/` — 仕様書 / 移行ロードマップ / 実装カタログ / 運用 / 教育資料 / `05_phase4_to_phase5_refactor_plan.md`
...
```bash
# Phase 5 全体を復元
git checkout pre-phase3to6-impl-removal -- 5/study-hybrid-search-vertex/

# 特定ディレクトリだけ復元
git checkout pre-phase3to6-impl-removal -- 5/study-hybrid-search-vertex/app/
