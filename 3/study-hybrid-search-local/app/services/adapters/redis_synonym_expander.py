...
correctness. This Phase 3 adapter is the same shape as the Phase 7
``app/services/adapters/redis_synonym_expander.py``; only the Redis
client is constructed against the Docker Compose ``redis://redis:6379/0``
endpoint instead of Cloud Memorystore.
"""
...
logger = logging.getLogger("app.services.adapters.redis_synonym_expander")
...
class RedisSynonymExpander(SynonymExpanderPort):
    ...
    sub-millisecond, dominated by RTT to the Redis instance.
    """
