...
def _reset_globals(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # tests never touch the real logs/step_timings.csv.
...
def test_step_done_emits_elapsed_line_matching_monitor_contract(
    ...
    # _step_done also appends a deploy-all row to the timing history CSV.
    assert step_timing.baselines("deploy-all").get("tf-bootstrap") is not None
